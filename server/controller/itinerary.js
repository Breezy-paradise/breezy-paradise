...
module.exports = {
    ...
    deleteItineraryItem: async (req, res) => {
        try {
            await req.app.get('db').user_itinerary.delete_itinerary(req.params.id, req.session.user.id);
    ...
    getLocationItinerary: async (req, res) => {
        try {
            const getUserItinerary = await req.app.get('db').user_itinerary.get_user_itinerary(req.params.location_id, req.session.user.id);
